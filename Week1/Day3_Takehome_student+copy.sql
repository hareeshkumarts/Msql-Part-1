...
select branch,sum(revenue) - sum(cost) profit
from bank_branch_pl
group by branch
having profit not like '-%';
...
select product,round(avg(estimated_sale_price),2) avg_price 
...
from bank_branch_pl  
group by branch
having (sum(revenue) - sum(cost)) > sum(Estimated_profit);
...
select branch , sum(estimated_profit) estimated_profit,sum(revenue) - sum(cost) Profit 
from bank_branch_pl 
group by branch;
...
select sum(revenue-cost) Loss , branch , Product, Banker
from bank_branch_pl
group by branch
having Loss like '-%';
...
from bank_branch_pl 
group by branch
having profit > 0 and min(profit);
...
select branch,Product, sum(revenue-cost) profit 
from bank_branch_pl
...
select product, (revenue - cost) as 'real_profit', estimated_profit from bank_branch_pl
 having real_profit < 110/100 * estimated_profit and real_profit > 90/100 * estimated_profit;
...
select product,branch, (revenue-(cost-(cost*0.3))) cal_profit
from bank_branch_pl
