...
select sum(Purchase_cost) sum_purchaseCost,round(avg(estimated_sale_price),2) avg_SalePrice from Bank_inventory_pricing where month =  2;

# Question 2:
# 2) Print average of estimated_sale_price upto two decimals from bank_inventory_pricing table.

select round(avg(estimated_sale_price),2) avg_sale_price from bank_inventory_pricing;
...
select sum(If(purchase_cost is NULL,2000,purchase_cost)) as sum_purchase_cost 
from bank_inventory_pricing;
...
select round(avg(ifnull(purchase_cost,estimated_sale_price)),2) from bank_inventory_pricing;

# Question 8:
# 8) Print the count of unique Products used in  Bank_inventory_pricing

select count(distinct product)  as unique_products_count from Bank_inventory_pricing;
...
from bank_inventory_pricing where purchase_cost is NULL
group by product;
...
select product,avg(Estimated_sale_price),avg(Estimated_sale_price) * 1.15
from bank_inventory_pricing
group by product
