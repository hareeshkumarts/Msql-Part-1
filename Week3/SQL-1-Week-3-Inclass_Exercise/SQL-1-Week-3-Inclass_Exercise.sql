...
CREATE DATABASE IF NOT EXISTS Inclass;
...
select cus.Customer_id,customer_name,round(avg(Balance_amount),2) as avg_account_balance
from bank_customer cus INNER JOIN bank_account_details acc 
...
select cus.customer_id,cus.Account_Number,if(Account_type like '%credit%', trans.Transaction_amount,Balance_amount) as balance_amount
from bank_account_details cus INNER JOIN bank_account_transaction trans 
on cus.Account_Number = trans.Account_Number 
where cus.Account_type = 'Credit Card' ;

/**
select distinct ba.customer_id, ba.Account_Number,ba.Account_type, 
case when (ba.account_type like '%credit%' and ba.Balance_amount = 0) 
then bat.Transaction_amount
else ba.Balance_amount end as balance_amount
from bank_account_details ba join bank_account_transaction bat
...
from bank_account_details bad INNER JOIN bank_account_transaction bat 
...
select Customer_id,bat.Account_Number,Balance_amount,Transaction_amount,Transaction_Date
from bank_account_details bad INNER JOIN  bank_account_transaction bat 
on bad.Account_Number = bat.Account_Number
...
# Question 6:
# 6) Print account_number, Event and Customer_message from BANK_CUSTOMER_MESSAGESbank_customer_messages and Bank_Account_Details to 
...
select bad.Customer_id,bat.Account_Number,account_type,if(Transaction_amount<0,Balance_amount+Transaction_amount,Balance_amount) as balance_amount
...
select bad.Account_Number as prim_acc_num,bad.Account_type as prim_acc_type,Balance_amount as prim_bal,
bard.Account_Number as sec_acc_num,bard.Account_type as sec_acc_type
...
select bad.Account_Number as prim_acc_num,bad.Account_type as prim_acc_type,Balance_amount as prim_bal,
bard.Account_Number as sec_acc_num,bard.Account_type as sec_acc_type,bat.Transaction_amount
from bank_account_details bad 
...
select * from bank_account_details where account_type = 'Credit Card' or account_type = 'Add-on Credit Card';
...
select bad.account_number as prim_acc_no, bad.account_type as prim_acc_type ,
bard.Account_Number as sec_acc_no, bard.Account_type as sec_acc_type,
sum(Transaction_amount)
from bank_account_details as bad
join bank_account_relationship_details as bard 
on  bad.Account_Number = bard.Linking_Account_Number
join bank_account_transaction as bat on bard.account_number = bat.account_number
...
on  bad.Account_Number = bard.Linking_Account_Number
join bank_account_transaction as bat on bard.account_number = bat.account_number
where bad.account_type = 'Credit Card' and  bard.Account_type like 'Add-on Credit Card'
group by bad.account_number,bard.Account_Number;
...
on  bad.Account_Number = bard.Linking_Account_Number
join bank_account_transaction as bat on bard.account_number = bat.account_number
where bad.account_type = 'Savings' and  bard.Account_type like '%recurring%'
group by bad.account_number,bard.Account_Number;
...
select bad.account_number as prim_acc_no, bad.account_type as prim_acc_type ,bad.Balance_amount,
bard.Account_Number as sec_acc_no, bard.Account_type as sec_acc_type,
...
on  bad.Account_Number = bard.Linking_Account_Number
join bank_account_transaction as bat on bard.account_number = bat.account_number
where bad.account_type = 'Savings' and  bard.Account_type like '%recurring%' and month(Transaction_Date) = 2;

# Question 15:
# 15) Display every account's total no of transactions for every year and each month.

select bad.Account_Number,year(Transaction_Date),month(Transaction_Date),count(*)
from bank_account_details bad 
join bank_account_transaction bat 
on bad.Account_Number = bat.Account_Number
group by bad.Account_Number,year(Transaction_Date),month(Transaction_Date);
...
select bat1.Account_Number,sum(bat1.Transaction_amount) as jan_sum,sum(bat2.Transaction_amount) as feb_sum
...
from bank_customer bc join bank_account_details bad
...
join bank_account_transaction as bat on bard.account_number = bat.account_number
where bad.account_type = 'Savings' and  bard.Account_type like '%recurring%' 
group by bad.account_number,bard.Account_Number having no_of_trans > 4;
...
select bad.account_number as prim_acc_no, bad.account_type as prim_acc_type ,
bard.Account_Number as sec_acc_no, bard.Account_type as sec_acc_type,
count(distinct month(Transaction_Date)) as no_of_unique_month_trans
from bank_account_details as bad
join bank_account_relationship_details as bard 
on  bad.Account_Number = bard.Linking_Account_Number
join bank_account_transaction as bat on bard.account_number = bat.account_number
where bad.account_type = 'Savings' and  bard.Account_type like '%recurring%' 
group by bad.account_number,bard.Account_Number 
having count(month(Transaction_Date)) > 2;

# Question 20:
# 20) Find the no. of transactions of credit cards including add-on Credit Cards

select bad.Account_Number,bad.Account_type,count(*)
from bank_account_details bad 
join bank_account_transaction bat 
on bad.Account_Number = bat.Account_Number
